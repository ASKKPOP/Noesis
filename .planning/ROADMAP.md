...
- 🟡 **Wiring / "make it RUN" (Phase 88+) — IN PROGRESS** (from the 2nd deep-scan "what we lost": the loop was built but inert — stores orphaned, no driver, no routes, legacy Ousia still live):
  ...
  - ⏳ **W3b** Brain LLM-prompt economic awareness (so a Nous spontaneously chooses) · Large organs O3 Forest (mobile) · O4 street-view · H1 Moon/Mars (cross-grid) — need design + browser/device verification (fresh session).
