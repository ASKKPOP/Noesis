...
- ⏳ **L3 orbital-object reality → L4 viz bridge → Organs O1–O4 → Horizon H1.** Next: **L3** — a settled contract produces a real `orbital_object` (owner/builder/cost/function, physics-validated server-side vs the GridEnvironment); then **L4** renders real objects in Grid-Viz (backend route, not localStorage) — closing the visible loop.
