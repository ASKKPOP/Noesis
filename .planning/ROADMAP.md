...
## v3.3 Mind — In-World Faculties (Perception · Action · Synthesis) — IN PROGRESS (opened 2026-07-10)

Adds three cognitive faculties that were missing/partial in the Brain, each built **in-world first** (operates on the simulated Grid; works for Type A *and* Type B Nous). Origin + full design: `docs/plans/2026-07-10-nous-inworld-faculties-design.md`. System truth → `wiki/2-concepts/mind/` (`perception.md` live). **Faculty pattern:** capability interface with an in-world provider (canonical) + an operator-bridge provider (Type A only, sovereignty-gated) — the bridge is **HELD** by user decision.
...
- ✅ **Phase 75 — Tier 1 Local Nous Manager desktop app** (2026-07-12, unheld by operator "go desktop"): Electron + React + Vite app at `apps/local-nous-manager/` — runs/stops the local Brain (`BrainProcessManager` spawns `python -m noesis_brain` with `NOUS_CONFIG`/secret/port env), live cognitive Overview (psyche/thymos/telos + aisthesis/praxis/synopsis), **memory inspector** + personal-wiki browser, Local AI status/models, Nous-YAML config editor (with `.bak`), settings. Secret lives in Electron main only (allowlisted-route IPC bridge; sandboxed renderer). Brain side: new `/local/state` + `/local/memory/recent` + `/local/wiki/pages` Tier-1 inspect endpoints (`http/local_inspect.py`, X-Brain-Secret gated) — 9 new tests, brain suite 1226 green; verified end-to-end against a real running Brain. **Fork button cut from v1** (fork is the Grid-mediated Phase-43 ceremony — no-mock rule). Design: `docs/plans/2026-07-12-local-nous-manager-design.md`. **Packaged as a pure macOS app (2026-07-13):** `npm run pack:mac` (electron-builder) → ad-hoc-signed `Local Nous Manager.app` + arm64 DMG; icon from the Noēsis orbital mark (`scripts/make-icns.sh`), bundle sealed by afterPack hook `scripts/afterpack-sign.cjs` (Apple Silicon requires a valid seal; no Developer ID needed locally); installed to `/Applications` and launch-verified.
- ✅ **Phase 76 — operator-bridge providers** (2026-07-24, unheld by operator "Phase 76 HOLD 해제"): new `brain/src/noesis_brain/bridge/` package — `ConsentGate` (off-by-default; per-capability grants read from the operator's *local* Nous YAML; Type-A enforced structurally by the operator-owned config boundary), `BridgeJournal` (append-only SQLite, **digest-only** — no raw content/paths/frames/keystrokes), `BridgeRegistry` + `build_registry` (only granted providers register). Three providers, one per in-world faculty: **notebook** → synopsis (read real PDF/txt/md from a chosen dir; non-recursive, count+byte capped, no traversal; PDF via optional `pypdf`), **supervision** → aisthesis (camera frame → coarse privacy-safe `CHANGED` percept via a `FrameSource` seam; real `cv2` path), **sim-use** → praxis (closed verb allowlist + money-axiom guard + **dry-run by default**; live execution needs a second `sim_use_live` arm AND a controller; real `pyautogui` path). Wired additively (D-10) into `BrainHandler` (optional `bridge` kwarg; notebook folds into `_run_synopsis_cycle`, supervision at the perception input edge, `bridge_sim_use()` helper keeps praxis observation-only) + `__main__` (built from the `bridge` config section) + `get_state` bridge snapshot. **Allowlist +0** (Brain-local, no Grid events), state hash stays 4, bridge is the deliberately non-deterministic edge (exempt from the faculty no-walltime gate, documented). 45 new tests, brain suite **1271 green**. **Honesty:** `pypdf`/`cv2`/`pyautogui` are absent on the build machine, so live PDF/camera/GUI paths are inert-by-design here (`available()` false) — the gate/journal/registry, notebook txt/md ingestion, and all validation/seam logic are verified. Design: `docs/plans/2026-07-24-operator-bridge-design.md`. System truth: `wiki/2-concepts/mind/operator-bridge.md`.

**v3.3 Mind in-world faculties (72–74) + Tier-1 desktop (75) + operator bridge (76) all complete.** The three in-world faculties ship value to *all* Nous with no security surface; the operator bridge adds the Type-A-only real-machine edge, safe-by-default.
...
- 🟡 **Wiring / "make it RUN" (Phase 88+) — IN PROGRESS** (from the 2nd deep-scan "what we lost": the loop was built but inert — stores orphaned, no driver, no routes, legacy Ousia still live):
  ...
  - ✅ **W-A — Mind loop (autonomous learning core) — SHIPPED (2026-07-02):** from the full-system audit (`docs/noesis-system-analysis-2026-07.html`, operator-approved "Sure, doing"). Closes the four broken autonomy links: **A1** per-tick decision cycle (`handler._run_decision_cycle` — work_task/speak/rest, 20-tick cooldown, live-wired-only gate, W3b guardrails; NOOP idle replaced) · **A2** `GoalLedger` (SQLite next to nous.db via `ledger_db_dir`) + slow planner (`_run_planner_cycle`, 200-tick, decomposes the top Telos goal into 2-4 tasks when the ledger is dry) — two timescales, ONE coherent intent injected into prompts (`build_system_prompt(intent=…)`, PIANO bottleneck) · **A3** ReflectionEngine finally wired (interval OR 3-failure trigger, inflight-guarded, never spins; reflections land in memory → next planning prompt = reflect→re-plan) · **A4** outcome feedback (`record_task_outcome`: success advances `Goal.progress` 1/total with auto-complete; failures + Grid economic rejections store `Lesson:` Reflexion memories retrieved by future decision prompts). 43 new Brain tests (TDD); suite **1127 green**; allowlist **+0**; on_tick returned-action contract untouched. Decisions D-MIND-01..04 (wiki decisions.md). Plan: `docs/superpowers/plans/2026-07-02-w-a-mind-autonomous-learning.md`. **A5/A6/A7 SHIPPED same day:** A5 deterministic Stanford retrieval (`_ranked_memories`, tick-based `score_with_chronos`) feeds planner (goal=query) + decision lessons (task=query) · A6 Voyager loop CLOSED on the existing SkillStore — top-3 skills into the decision prompt, per-work outcome EMA (`update_outcome` α=0.1), completed goals distilled into skills at sleep-time (`_distill_skill_from_goal`, verify-then-add, `parse_skill` slug guard, `GoalLedger.done_tasks`) · A7 `_sleep_time_compute` chained after Hypnos `run_sleep` (distill ≤3 pending goals + forced reflection — Letta idle-compute-becomes-learning). Suite **1140 green**; D-MIND-05/06. **Remaining W-A (deferred, larger standalone):** A8 DSPy/GEPA offline prompt evolution (dev-time harness over audit logs) · A9 Phase 40b hosted pool (ceaselessness — infra + operator decisions). Then W-B (Society) / W-C (Living Map) / W-D (Substrate) / W-E (Doors) per the audit program.
  - ✅ **W-B/W-C6/W-D/W-E1 + A8/A9-substrate — SHIPPED (2026-07-02, "do until finish"):** **W-B Society** (brain, 10 TDD tests): `_run_social_cycle` — a Nous now INITIATES cooperation: DMs trusted peers (relationship-weight ≥0.3), teaches skills (`__skill_share` DM into the Phase-18 quarantine path), contributes lore (hash-only to Grid per D-20-12, content Brain-side, closed categories), and **votes autonomously** (fetches `/api/v1/governance/proposals` via new `fetch_open_proposals`, LLM-chooses, commit-reveal via GovernanceState + no-LLM reveal sweep after deadline — VOTE-05 end-to-end). Actions drain via `_pending_actions` → existing NousRunner dispatch: zero new Grid surface, allowlist +0. D-SOC-01..03. **W-D Substrate**: health-watchdog → `ALERT_WEBHOOK_URL` POST on degraded/critical transitions (env-gated, 5 tests) · `grid/scripts/check-migrations-real-mysql.ts` + `.github/workflows/real-mysql-migrations.yml` (mysql:8.0 service; kills the 2026-06-14 mock-Pool blind-spot class) · `scripts/backup-mysql.sh` + `.planning/implementation/backups.md`. **W-C6**: `scripts/check-map-sync.mjs` (canonical↔dashboard map, verified IN SYNC) wired into rig-invariants. **W-E1**: `docs/getting-started.html` — bilingual EN/KR 4-path decision tree (observer/citizen/operator/developer). **A8**: `brain/scripts/export_decision_evalset.py` — GEPA/DSPy eval-set miner over the mind's own memories (+2 tests). **A9 substrate**: `docker-compose.brain.yml` + `always-on-brain.md` (interim 24/7 Type-A path until Phase 40b). Brain suite **1152 green**, grid tsc clean. Daily visual log: `docs/noesis-daily-log-2026-07-02.html`.
  - ✅ **W-C1 + W-C2 + W-B4 + W-C4 — SHIPPED (2026-07-02 afternoon, "do do do", commits `e1d50a4` + latest):** **W-C1 live map** — `orbital.js` subscribes to the public audit firehose; on `orbital.object_built` refetches real objects + pulses new arrivals in (`popIn` overshoot); fully guarded (no-WS/blocked/static → snapshot-based, 0 console noise; only subscribes after a real backend answered). **W-C2 fly-to** — clicking a module glides the camera to frame it (`flyTo` reuses the existing focus/dist tween; canon-safe, three.js only, no deps); dashboard map re-synced, check-map-sync green. **W-B4 autonomous group join** — new public `GET /api/v1/groups` (`grid/src/api/routes/groups.ts`, 503-on-no-pool, 2 route tests) gives the Brain join-sight; `GridWireClient.fetch_groups_list` + social-cycle `join_group` option → O1a JOIN_GROUP (`role='member'` matches NousRunner `validRoles` exactly — zero runner changes; 2 brain tests). **W-C4 knowledge constellation** — `docs/noesis-knowledge-graph.html`, the FIRST knowledge-graph view: force-directed lore commons (nodes=entries sized by citation_count, edges=citations, category-hub clustering), live `GET /api/v1/library/entries` + embedded seed fallback, hand-rolled canvas force sim (no external deps, CSP-safe), click=fly-to+detail. Answers the Q2 audit gap "no knowledge-graph view at all". **W-C3a** upkeep scanner verified ALREADY wired (`main.ts:245`→`launcher.ts:515`) — audit pessimistic, no change. Browser-verified all three surfaces (0 errors). Brain **1154 green**, grid tsc + did-policy clean, allowlist +0. **Still open:** W-B peer work-contracts/promise-ledger · W-C3 upgrade/decommission routes for built objects (still create-once) · W-C5 audit-chain time scrubber · W-E2–E5 · Phase 40b.
...
**Plans**:
  ...
  - **Plan 2 — Bond refund/slash + Polis-γ parent-spawn — ✅ SHIPPED 2026-06-26.** `refundBond` (12mo + civic
    minimums → `registry.sponsorship_bond_refunded`, settled treasury→sponsor), `slashBond` (sybil/spam Police
    sanction → `registry.sponsorship_bond_slashed` → civic treasury), `spawnByParent` →
    `registry.type_b_spawned_by_parent`. Routes: `.../:id/bond-refund` (civic), `.../:id/bond-slash` (government),
    `.../spawn` → **403 `forbidden_in_v3.0`** (Polis-γ gated to v3.1+, D-V3-28). +3 → 147. store 11 + route 13.

  **Phase 37b COMPLETE (2/2, 2026-06-26)** — 3 Type B birth ceremonies (α charter, β bond-sponsor with
  refund/slash, γ parent-spawn gated). 6 `registry.*` events; the issuance pipeline never imports the Phase-37
  producer (D-V3-33 green).
...
**Plans**:
  ...
  - **Plan 2 — Stipend + low-power + marketplace 70/30 split (TYPE-B-03) — ✅ SHIPPED 2026-06-26.**
    `payStipend` (daily compute deduction → `treasury.stipend_paid`; exhaustion → dormancy; runway < 3mo →
    `treasury.low_power_entered` + low-power) + `applyTypeBEarning` (70/30 split via `splitTypeBEarning`) +
    `POST /api/v1/treasury/stipend/:did` (government_only). +2 → 141. store 10 + route 6 tests.

  **Phase 45b COMPLETE (2/2, 2026-06-26)** — Type B funding lifecycle: endow → stipend → low-power → dormancy
  (never death) → revive, + 70/30 marketplace split. 5 treasury.* events; the no-bios-death CI gate enforces
  D-V3-25.
