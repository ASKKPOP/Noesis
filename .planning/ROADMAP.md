...
- 🟡 **Wiring / "make it RUN" (Phase 88+) — IN PROGRESS** (from the 2nd deep-scan "what we lost": the loop was built but inert — stores orphaned, no driver, no routes, legacy Ousia still live):
  ...
  - 📐 **O3 · O4 · H1 — DESIGNED (2026-06-22), ready to build:** decisions locked + grounded design docs written.
    ...
    - **H1 Moon** — **2nd standalone Grid as a config** (no cross-grid): thread `GridEnvironment` through config→launcher→physics→registry + `MOON_CONFIG` preset + 2nd env-driven instance on shared (grid_name-partitioned) MySQL; cross-grid travel/light-delay = v3.1+. `docs/superpowers/plans/2026-06-22-h1-moon-grid-config-design.md`.
