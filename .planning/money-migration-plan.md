...
| A′ | **62.5 — In-DB ledger unification** *(precursor)* | Unify the in-DB economy on `nous_accounts` (civic-DID) + `civic_treasury`; retire `nous_registry.balance_wei` as money **and the birth faucet (ECON-01)**; reconcile treasury. Fixes the split-ledger bug ([Issue #9](https://github.com/ASKKPOP/Noesis/issues/9)) so Phase 63 bridges **one** coherent balance. See [phases/62.5-indb-ledger-unification/62.5-CONTEXT.md](phases/62.5-indb-ledger-unification/62.5-CONTEXT.md). | ✅ **COMPLETE 2026-07-11** — all waves 01–05. 04 (PR #16) + 05 (PR #17): `transferWei` removed, faucet zeroed (v74), slash→fine-to-treasury, `did:noesis:system:treasury` record retired (v75), CI gate `check-ledger-b-money.mjs`, Issue #9 E2E green. Precursor 62.6 (PR #12) migrated the 4 subsystems. **Issue #9 closed.** |
| A″ | **62.6 — Ledger-B subsystem migration** *(precursor)* | Migrate the four remaining Ledger-B subsystems — **marketplace** (+skim), **upkeep**, **co-work/co-build**, **agent-trades** — off `nous_registry.balance_wei`/`transferWei` onto `nous_accounts`/`civic_treasury`. Discovered during 62.5-03: the leftover Ledger-B money is whole subsystems, not stray sites (operator: "do not migrate blind", 2026-07-10). **Unlocks 62.5-04** (fold + remove `transferWei`) **and 62.5-05** (retire `did:noesis:system:treasury` record, delete PR #8/#11, CI gate). See [phases/62.6-ledger-b-subsystem-migration/62.6-CONTEXT.md](phases/62.6-ledger-b-subsystem-migration/62.6-CONTEXT.md). | ✅ **COMPLETE 2026-07-10** — 5/5 plans, verify 8/8, review clean. Last `transferWei` money site retired; `did:noesis:system:treasury` has no writer. D-13 operator decision: agent-trades citizens-only (existence→civic, either null ⇒ `trade.rejected{not_found}`, no allowlist change). Suite 4101 green + all invariant gates. |
