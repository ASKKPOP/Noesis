...
| A′ | **62.5 — In-DB ledger unification** *(precursor)* | Unify the in-DB economy on `nous_accounts` (civic-DID) + `civic_treasury`; retire `nous_registry.balance_wei` as money **and the birth faucet (ECON-01)**; reconcile treasury. Fixes the split-ledger bug ([Issue #9](https://github.com/ASKKPOP/Noesis/issues/9)) so Phase 63 bridges **one** coherent balance. See [phases/62.5-indb-ledger-unification/62.5-CONTEXT.md](phases/62.5-indb-ledger-unification/62.5-CONTEXT.md). | ◑ waves 01–03 done; **04/05 now UNBLOCKED** by 62.6 (2026-07-10) |
