...
### Reference-UI Integration (Area 6 — added 2026-05-25 post-UI-SPEC)

User shared a visual reference UI for Portal landing page after UI-SPEC was approved. The reference confirmed most locked decisions and introduced 4 additional locks integrated here.

- **D-36-21:** Auth methods extended — **Google OAuth + Apple OAuth added** as visitor-friendly options alongside SIWE + email. D-V3-15 amended: no-DID write exception endpoints grow from **3 → 5** (`/portal/auth/siwe` + `/portal/auth/email/signup` + `/portal/auth/email/signin` + `/portal/auth/oauth/google` + `/portal/auth/oauth/apple`). CI gate `scripts/check-no-did-exception-count.mjs` updated to assert 5 (not 3). Rationale: lowers signup friction for mainstream humans; OAuth providers DO see "user signed up at Noēsis Portal" but cannot read Grid state. Sovereignty principle preserved (operator-DID still derived locally; OAuth is identity provider, not custodian).
- **D-36-22:** Civic terminology **adopts "Grid Charter" + "Laws of Themis"** as named civic instruments. **Grid Charter** = immutable founding document published at Grid creation (set by Polis founding members; cannot be amended post-creation). **Laws of Themis** = changeable body of Polis-legislated bills (bills become "Laws of Themis" upon `gov.law_enacted`). Themis (the ethics Brain process from v2.6) becomes the named source/spirit of civic law — a mythological framing that ties cognitive architecture to civic structure. Visitor terms-of-service copy: "By entering Genesis Grid, you agree to the Grid Charter and the Laws of Themis." (verbatim).
- **D-36-23:** Portal landing page **is Dashboard-class for visual purposes** — 3D libs (three.js, aframe, etc.) permitted for the isometric city background visual on Portal landing only. D-V3-06 raw-SVG invariant **continues to apply to**: Steward Console (all surfaces), Civic Map functional view (per UI-SPEC §Surface 2), Polis surfaces, Library, Marketplace, Public Profile. Marketing surface (Portal landing hero) can use 3D; functional surfaces stay flat SVG.
- **D-36-24:** Portal landing optional **atmospheric toggle controls** ("NIGHT / PACKETS / RAIN" per reference UI) are **deferred to v3.1** polish. Phase 36 ships Portal landing functional only; atmospheric effects are aesthetic, not architectural. If included in v3.0, they're a Phase 56 (Portal user UI) follow-up, not Phase 36 scope.
- **D-36-25:** Visitor-visible Grid stats footer (reference UI shows "ACTIVE NODES · PACKETS/S · GRID HEALTH · UPTIME") is **scoped to safe metrics only**: `GRID HEALTH` (computeStatus output per D-32-C2 — already public via /health) and `UPTIME` (since last restart — safe). **EXCLUDED** (Q-VA-3 fingerprinting concern): `ACTIVE NODES` (leaks Type B census per D-V3-24 cap), `PACKETS/S` (leaks audit event volume — already addressed by D-V3-13 visitor_count internal-only). v3.0 visitor footer: just `GRID HEALTH: <status>` and `UPTIME: <h:m:s>`. v3.1 may revisit after Type B population stabilizes.
