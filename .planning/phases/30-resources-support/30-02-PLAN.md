...
goal: Create /portal/help/faq with ~20 Q&A pairs organized in categories, a CSS-details accordion, and a client-side search/filter
type: execute
wave: 1
depends_on: []
files_modified:
...
must_haves:
  truths:
    - "/portal/help/faq renders ~20 FAQ pairs organized in at least 4 categories"
    - "Accordion uses <details>/<summary> — no JavaScript library"
    - "Client-side search filters visible Q&A pairs as user types"
    - "Page has a breadcrumb 'Help / FAQ'"
  artifacts:
    - path: dashboard/src/app/portal/help/faq/page.tsx
      provides: "FAQ page with accordion categories and client-side search"
      contains: "Wallet & Auth"
    - path: dashboard/src/app/portal/help/faq/page.tsx
      provides: "Client-side search filter"
      contains: "use client"
  key_links:
    - from: "dashboard/src/app/portal/help/faq/page.tsx"
      to: "/portal/help"
      via: "Breadcrumb link back to Help Center"
      pattern: "href.*portal/help"
---

<objective>
Create `/portal/help/faq/page.tsx` — the full FAQ page with accordion categories, ~20 Q&A pairs, and client-side search. This is the expanded replacement for the 8-entry FAQ in the old `/portal/help` placeholder.

Purpose: Users need self-serve answers to common questions. The existing help/page.tsx had only 8 Q&As; this page delivers ~20 covering all four core categories.

Output: New `faq/page.tsx` directory + page file, fully static except for the search filter hook.
</objective>
...
<!-- Existing 8 FAQ entries from help/page.tsx — incorporate all 8 into the new page -->
Category: Wallet & Auth
- "What is Noēsis?" / "Noēsis is a persistent AI society..."
- "How do I sign in?" / "Connect your Ethereum wallet..."
- "What is a DID?" / "A Decentralised Identifier..."
- "What is SIWE?" / "Sign-In With Ethereum (EIP-4361)..."

Category: Nous & Agents
- "What can I do as an H1 observer?" / "Watch the World Map, read Nous activity..."
- "What is a Tick?" / "A Grid heartbeat — approximately every 30 seconds..."
- "When does Chat with Nous launch?" / "Phase 26. You will be able to send messages..."

Category: Economy (Cyber Coin)
[Add ~5 new entries]
...
<task type="auto">
  <name>Task 1: Create /portal/help/faq directory and page with accordion + client-side search</name>
  <files>dashboard/src/app/portal/help/faq/page.tsx</files>
  <action>
Create the directory `dashboard/src/app/portal/help/faq/` and write `page.tsx`.

This page uses `'use client'` because it has a search `useState`. Import `{ useState }` from React.

**FAQ data** — hardcode as a const array of categories with Q&A pairs:
...
Write exactly this page (adapting slightly for readability). The FAQ data is hardcoded — no DB fetch.
  </action>
  <verify>
    <automated>cd /Users/desirey/Programming/src/Noesis/dashboard && npx tsc --noEmit 2>&1 | head -20</automated>
  </verify>
  <done>
    - File `dashboard/src/app/portal/help/faq/page.tsx` exists
    - `grep -c "use client" dashboard/src/app/portal/help/faq/page.tsx` returns 1
    - `grep -c "useState" dashboard/src/app/portal/help/faq/page.tsx` returns at least 1
    - `grep -c "details" dashboard/src/app/portal/help/faq/page.tsx` returns at least 4 (one per FAQ pair)
    - `grep -c "Wallet" dashboard/src/app/portal/help/faq/page.tsx` returns at least 1 (category label)
    - `grep -c "Cyber Coin" dashboard/src/app/portal/help/faq/page.tsx` returns at least 1 (category label)
...
|----------|-------------|
| Client browser → FAQ page | User types into a filter input; result is client-side only, no data sent to server |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-30-02 | Information Disclosure | /portal/help/faq | accept | Entirely static editorial content; search is client-side filter only; no user data crosses any boundary |
</threat_model>

<verification>
- File exists at `dashboard/src/app/portal/help/faq/page.tsx`
- Page has `'use client'` directive (required for useState search filter)
- Uses `<details>`/`<summary>` for accordion (no JS library)
- TypeScript compiles
- ~20 Q&A pairs across 4 categories present
</verification>

<success_criteria>
- /portal/help/faq page renders FAQ accordion with 4 categories and ~20 Q&A pairs
- Search input filters visible questions in real time (client-side only)
- Breadcrumb "Help / FAQ" with link back to /portal/help is present
- No backend calls — fully static content
</success_criteria>

<output>
After completion, create `.planning/phases/30-resources-support/30-02-SUMMARY.md` documenting:
- Number of FAQ pairs written
- Any deviations from planned FAQ content
</output>
