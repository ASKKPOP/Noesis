...
goal: Create /portal/help/glossary with all Noēsis terms, anchor links, alphabetical jump nav, and cross-term references
type: execute
wave: 1
depends_on: []
files_modified:
...
must_haves:
  truths:
    - "/portal/help/glossary renders all key Noēsis terms with 2-3 sentence definitions"
    - "Each term has an anchor ID (e.g., #nous, #ousia) for direct linking"
    - "Alphabetical jump-to nav at top of page"
    - "Each term links to related terms"
    - "Breadcrumb 'Help / Glossary' with link back to Help Center"
  artifacts:
    - path: dashboard/src/app/portal/help/glossary/page.tsx
      provides: "Full glossary with anchor IDs and letter nav"
      contains: "id=\"nous\""
    - path: dashboard/src/app/portal/help/glossary/page.tsx
      provides: "Alphabetical letter jump navigation"
      contains: "jump"
  key_links:
    - from: "dashboard/src/app/portal/help/glossary/page.tsx"
      to: "/portal/help"
      via: "Breadcrumb link"
      pattern: "href.*portal/help"
---

<objective>
Create `/portal/help/glossary/page.tsx` — the expanded Noēsis term glossary with anchor IDs, alphabetical letter navigation, and cross-references between related terms. Incorporates and expands the 16-term set from the old `/portal/glossary/page.tsx`.

Purpose: The glossary is a cornerstone reference document for new users. It explains the unique Noēsis lore vocabulary that users encounter throughout the portal.

Output: New `dashboard/src/app/portal/help/glossary/` directory and `page.tsx` server component.
</objective>
...
@dashboard/src/app/portal/glossary/page.tsx
@dashboard/src/app/portal/privacy/page.tsx

<interfaces>
<!-- Existing 16 terms from /portal/glossary/page.tsx — include all, expand definitions -->
Nous, Ousia, Genesis Grid, Agora, DID, SIWE, Lore, Norm, Whisper, Tick, Sophia, Hermes, Themis, Cyber Coin, Agency Tier, Allowlist

<!-- Additional terms to add (from roadmap and state.md lore) -->
Psyche, Brain, Telos, Creed, AuditChain, Skill, Iris, Nous Registry, Spawn, Region, Grid

<!-- CSS variables -->
--parchment, --rule, --ink, --muted, --bronze, --serif, --sans-portal, --mono-portal
...
<task type="auto">
  <name>Task 1: Create /portal/help/glossary with full term set, anchors, and letter nav</name>
  <files>dashboard/src/app/portal/help/glossary/page.tsx</files>
  <action>
Create directory `dashboard/src/app/portal/help/glossary/` and write `page.tsx` as a server component (no `'use client'`).

**Term data** — a const array of objects `{ term, slug, def, related? }`. Use the following 25 terms (alphabetical by term):
...
**Letter navigation** — extract unique first letters from slugs and render as an anchor row:
...
export default function GlossaryPage() {
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: 760 }}>
      ...
      {/* Term list — group by first letter */}
      {letters.map(letter => {
...
This is a server component — no `'use client'`. Use `<dl>` semantics (dt/dd) inside the container divs. Use plain `<a>` for anchors since this is server-rendered and Next.js Link is not needed for in-page anchors.
  </action>
  <verify>
    <automated>cd /Users/desirey/Programming/src/Noesis/dashboard && npx tsc --noEmit 2>&1 | head -20</automated>
  </verify>
  <done>
    - File `dashboard/src/app/portal/help/glossary/page.tsx` exists
    - `grep -c "use client" dashboard/src/app/portal/help/glossary/page.tsx` returns 0 (server component)
    - `grep -c 'id="nous"' dashboard/src/app/portal/help/glossary/page.tsx` returns at least 1
    - `grep -c "SEE ALSO" dashboard/src/app/portal/help/glossary/page.tsx` returns at least 1
    - `grep -c "letter-" dashboard/src/app/portal/help/glossary/page.tsx` returns at least 2 (letter nav anchors)
...
|----------|-------------|
| Client browser → glossary page | Fully static server-rendered page; no user data, no dynamic queries |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-30-03 | Information Disclosure | /portal/help/glossary | accept | Pure static editorial content; no user data; no auth gate required (glossary is public reference) |
</threat_model>

<verification>
- File exists at `dashboard/src/app/portal/help/glossary/page.tsx`
- No `'use client'` — pure server component
- At least 25 terms with anchor IDs
- Letter jump navigation rendered
- Cross-references between terms via anchor hrefs
- TypeScript compiles
</verification>

<success_criteria>
- /portal/help/glossary renders all Noēsis terms with 2-3 sentence definitions
- Each term has a unique anchor ID (e.g., id="nous") for direct deep-linking
- Alphabetical letter jump nav at top of page
- "See Also" cross-links between related terms
- Breadcrumb "Help / Glossary" with link back to /portal/help
</success_criteria>

<output>
After completion, create `.planning/phases/30-resources-support/30-03-SUMMARY.md` documenting:
- Final term count in the glossary
- Any terms added or modified beyond the plan
</output>
