...
- **Wall-clock grep gate:** `scripts/check-wallclock-forbidden.mjs` Tier A extended with `brain/src/noesis_brain/hypnos/` path. CI enforced alongside bios/ and chronos/ (T-16-03 closed).
- **Zero-diff invariant extended to Phase 16:** Two HypnosRuntimes with identical episodes produce byte-identical snapshot hashes. `test_zero_diff_sleep_actions_are_additive` + `test_compute_snapshot_hash_stable_across_calls` in `brain/test/hypnos/test_zero_diff.py` (HYP-04 closed).
- **T-16-02 discipline enforced:** `HypnosRuntime.run_sleep` is `async def`; `handler.py` uses `asyncio.create_task(_run())` pattern (inner coroutine wraps `await rt.run_sleep()`); no direct `await run_sleep` at `on_tick` scope. Verified by `brain/test/hypnos/test_sleep_trigger.py` structural source-inspection tests.
