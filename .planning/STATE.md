...
**Economic Reality Loop program (opened 2026-06-21, Phases 80+) now sequences this work.** It closes the loop civic due → treasury → Polis RFP → Nous bid → build → wei payout → real orbital object → rendered. The money rails are unit **F1** (model-first/chain-ready), and the IRS-tax open question is resolved by **D-MONEY-08** (civic due — overturns D-V3-22: the treasury fills from transaction fees **+ a recurring civic due** payable in labor or ETH). Foundation-first multi-planetary: every Grid carries a `GridEnvironment` so Moon/Mars are configs, not rewrites. **F0 + F0b SHIPPED** (F0: browser GridEnvironment + body-parameterized physics gate, node 53/53, browser-verified; F0b: grid-side `GridEnvironment` on `GridRecord` + Portal discovery feed, vitest 10/10, tsc clean). Next: **F1 money rails**, then **L1** (civic due + D-MONEY-08). Design + plan: `docs/superpowers/specs/2026-06-21-noesis-economic-reality-loop-design.md` (+ `.html`), `docs/superpowers/plans/2026-06-21-f0-gridenvironment-physics-gate.md`. This is a **parallel program**; the civic v3.0 milestone focus below (Phase 47 Police) is unchanged.
