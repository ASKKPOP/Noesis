...
**Current focus:** v3.3 Mind — In-World Faculties **COMPLETE (Phases 72–74, 2026-07-10)**: ✅ `aisthesis` (perception, input edge) + ✅ `praxis` (action, output edge) + ✅ `synopsis` (research synthesis, background). Faculties + on_tick hooks + get_state; brain suite **1217 green**, state-hash still closed at 4, no allowlist change. Phases 75–76 (operator bridge + Tier 1 Local Nous Manager + Type-A providers) **HELD by operator**. Design: `docs/plans/2026-07-10-nous-inworld-faculties-design.md`. (Parallel track still open: v3.2 Money 62.5-04/05 per stopped_at pointer above.)
