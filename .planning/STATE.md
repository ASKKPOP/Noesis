...
stopped_at: >-
  Phase 76 (operator bridge) COMPLETE 2026-07-24 (branch `plan/phase-76-operator-bridge`, pushed).
  Unheld by operator ("Phase 76 HOLD 해제"). New `brain/src/noesis_brain/bridge/` package: ConsentGate
  (off-by-default, per-cap grants from operator-local YAML, Type-A structural), BridgeJournal
  (append-only SQLite, digest-only), BridgeRegistry + build_registry. Three providers, one per
  in-world faculty — notebook→synopsis (real docs), supervision→aisthesis (camera→coarse percept),
  sim-use→praxis (allowlist + money-guard + dry-run-by-default + second live arm). Wired additively
  (D-10) into BrainHandler (notebook folds into `_run_synopsis_cycle`; supervision at perception input
  edge; `bridge_sim_use()` keeps praxis observation-only) + `__main__` (`bridge` config section) +
  get_state snapshot. Allowlist +0, state hash stays 4, bridge exempt from the no-walltime gate by
...
🔒 **SECURITY QA ROUND (2026-07-10) — 3 pre-existing auth findings fixed** (3-agent audit beyond the diff; user asked "test/qa security, very important"). None were introduced by the D-SEC-04 fix; all found by auditing the whole auth surface. Verified: full grid tsc clean (pre-existing @aws-sdk only), all security CI gates green, api regression 699 + police/governance/db stores 118 pass. +0 broadcast-allowlist.
  - **D-SEC-06 (CRITICAL — Brain-JWT confused-deputy):** `tryDid` accepted any `sub` (civic-DID) on a signature-valid Brain JWT → impersonate any Nous on every `civic_did_required` route. Fix: bind `sub` to `iss` via `civic_did_registry.getByExistenceDid` (source of truth); enforced-when-registry-wired (prod always wires it, main.ts). tryDid + server.ts (2 call sites) + `TryDidServices`; unit tests: impersonation/no-binding/revoked → null.
  - **D-SEC-05 (HIGH — public Brain-key hijack + un-revoke):** `POST /brain/token/register` (public, self-signed) `upsert`ed → overwrite any brain's key + reset `revoked=0` (defeats government court-order revoke). Fix: `insert` (INSERT IGNORE) — first-registration-only; rotation moves to a Portal-gated path (follow-up). Regression test locks no-overwrite/no-un-revoke.
  - **D-SEC-07 (MEDIUM — Police sanction not bound):** v3.0 has no Police role; `police_only` fell through to `civic_member`, and `execute-sanction` let any member pick an arbitrary sanction (freeze/fine/amount) against a convicted party. Fix: `execute-sanction` applies ONLY `charge.recommended_sanction` (body ignored; `fine` amount `null` — no arbitrary treasury credit, exact-amount binding is a follow-up); `police_only` relabeled honestly → `civic_did_required` (community-policing; conviction stays `government_only`).
  - **Verified SOUND (no change needed):** Portal cookie ES256 (alg-confusion-safe), Government session (ES256, fixed `iss`, not a shared secret), Operator allowlist (fail-closed), Admin routes (`operatorTierGate` + `GRID_ADMIN_ENABLED`), WS routes (`GRID_WS_SECRET`), Steward proxy (env creds, no client-header forward), default-deny coverage, policy-key↔route match.
  - **Open follow-ups (tracked):** authenticated Brain-key rotation path (D-SEC-05); fixing the exact Police fine amount/duration at conviction time so `fine` regains monetary teeth (D-SEC-07). Both are latent-until-write-back-milestone (brain_tokens + civic_did_registry currently EMPTY in prod).
  - **DEPLOYED to prod 2026-07-10** (commit `a9dfa1f5`, `./deploy.sh grid` with `NOESIS_DEPLOY_KEY=~/.ssh/sierrabiolab-aws.pem` — the deploy.sh default key path is wrong). Prod fast-forwarded `a4d8bf9→a9dfa1f` (also carried the already-merged D-SEC-04 governance fix `fe9942c8` + PR #11 registry census). +0 migrations (code-only rebuild). Verified: `noesis-grid` container healthy, internal `/health` 200, public `https://api.noesiis.com/health` → 200 `{"status":"ok"}`, `https://noesiis.com` 200. The whole `x-operator-tier` header-trust class + the 3 QA-round auth findings are now closed in production.
...
4. ✅ **Q3 availability + beginner — SHIPPED** (`4ddef27` + merge, branch `feat/q3-availability`): **(a) email+SMS alerting via AWS SNS** (operator #9): `HealthWatchdog` gains a second env-gated channel — on a degraded/critical transition it publishes `{grid,status,reason,tick}` to `ALERT_SNS_TOPIC_ARN` (region `ALERT_SNS_REGION`/`AWS_REGION`), fanning out to email+SMS subscribers. `@aws-sdk/client-sns` dynamic-imported only when configured (zero cost unused); fire-and-forget, never blocks the loop. 2 tests (mock the dynamic SDK import), watchdog suite 23 green. Runbook `.planning/implementation/alerting.md` (create topic → subscribe email+SMS → IAM publish perm → env). **(b) beginner on-ramp:** `scripts/quickstart.sh` collapses ~10 manual steps to 3 (install Docker+Ollama → run → open browser) — checks Docker+Ollama, pulls the model, generates `BRAIN_HTTP_SECRET` into `.env`, then `docker compose up` the full stack; `getting-started.html` leads the citizen path with this one-command route (bilingual). MySQL kept per operator.
