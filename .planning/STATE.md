...
**Current focus:** v3.3 Mind. ✅ Phases 72–74 in-world faculties (aisthesis/praxis/synopsis) shipped 2026-07-10 + merged (PR #13) + deployed. ✅ **Phase 75 Tier-1 Local Nous Manager desktop app** shipped 2026-07-12 (`apps/local-nous-manager/` Electron+React+Vite; Brain `/local/*` inspect endpoints; brain suite **1226 green**; e2e-verified against a real Brain; fork button cut per no-mock rule — fork stays the Phase-43 ceremony; **packaged 2026-07-13 as a pure macOS app** — electron-builder arm64 DMG + ad-hoc-sealed `Local Nous Manager.app`, icon from the orbital mark, installed to `/Applications`). Phase 76 (operator-bridge providers: sim-use/supervision/notebook) **STILL HELD**. Designs: `docs/plans/2026-07-10-nous-inworld-faculties-design.md`, `docs/plans/2026-07-12-local-nous-manager-design.md`. (Parallel track still open: v3.2 Money 62.5-04/05 per stopped_at pointer above.)
