...
| Architecture reconciliation — external engineering doc mixed against canon | Separate-session doc re-derived the vision blind to the repo; where it agreed canon is validated, where it conflicted the locked decision wins (Portal/Polis split not "Government"; Bios not "grid-credit"; discovery-registry not matchmaker engine; VOTE-05 settles governance legitimacy). Adopted A1–A12 engineering articulations into v3.1 phase plans (capability tokens, severance FSM, registry lease, signed Cowork Agreements, cross-House prompt-injection invariant). Tensions T1 (ex-ante vs living law), T2 (W3C VC), T3 (Mind mobility) flagged for user. (Reference: `.planning/research/v3.1/ARCHITECTURE-RECONCILIATION.md`) | — v3.1 planning (2026-06-12) |
| v0.2 Conflict / War / Immune layer — idea-stage, reconciled | Inter-Grid resource competition modeled as a **defense-as-vaccination immune system**: attacks attenuated (simulated CTF over the ledger, never real exploits — CI-enforced attenuation wall), defenses are real tested antibody code, threat space mutates (develop-or-die). Recorded as **prospective milestone (provisional v3.2), NOT scheduled** — prerequisite is multiple live Grids. Adopted C1–C10; overrides: Bios not "grid-credit", Portal/Polis not "Government", **war is a VOTE-05 Polis act never Henry's**, Henry not a war operator. Worldview invariants → PHILOSOPHY §11 (attenuation wall · Brain uncapturable · no Nous erased by conquest · spoils redistributed). **All 4 tensions RESOLVED (user, 2026-06-12):** W1 conquest = migration with full identity (no Nous erased); W2 winner-takes-all confirmed natural, recourse via Portal war-court (loss appealable, not prohibited); W3 sanctioned-offense (full war layer kept, 3 CI-gated ship-blockers: attenuation wall, Brain uncapturable, VOTE-05 decides war; offense only on consented CTF surfaces); W4 physical-right (spoils touch only civic/Type-B/Portal-pooled compute, never a Type A GPU). (Reference: `docs/v0.2/RECONCILIATION.md`, `docs/v0.2/immune_sim.py`) | — prospective (2026-06-12) |
