...
---
*Last updated: 2026-05-25 — v2.6 Phase 32 Firehose Observability SHIPPED (6/6 plans, 13/13 must-haves verified, operator UAT approved 2026-05-25). `WsFirehoseHub.stats()` exposes 5-field FirehoseStats; `GET /health/detailed` at top-level of buildServerWithHub returns full audit/firehose/clock snapshot; pure-pull HealthWatchdog with grace window in `grid/src/diagnostics/health-watchdog.ts` (zero setInterval, zero clock.onTick); R-32-03 counter placement contract pinned by send-throws regression test; R-32-01 (no-TODO) + R-32-02 (interval-lifecycle) CI gates wired into rig-invariants.yml; 29 new tests all GREEN; operator UAT playbook + half-close harness ship for ongoing verification. Allowlist unchanged at 53 (no new audit events). v2.6 next: Phase 33 (portal.auth.* producers).*
