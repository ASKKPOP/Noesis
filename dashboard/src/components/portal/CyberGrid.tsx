...
/**
 * CyberGrid — isometric city canvas.
 *
 * Extracted from the original portal/page.tsx so it can be used
 * as a full-screen background on the auth page (and standalone at /grid/world-map).
 *
 * Renders:
 *   - Isometric tile + building grid with 7 district types
 *   - Animated data-packet arcs between hub nodes
 *   - Matrix rain effect (katakana characters)
 *   - Ambient floating particles
 *   - HUD overlays: header, stats bar, district legend, controls
 *   - Hover tooltip and click-to-expand district panel
 *
 * Usage:
 *   import dynamic from 'next/dynamic';
 *   const CyberGrid = dynamic(() => import('@/components/portal/CyberGrid'), { ssr: false });
 *   <CyberGrid />
 *
 * Security: no dangerouslySetInnerHTML, no eval. Canvas-only rendering.
 */
...
export default function CyberGrid() {
    ...
    useEffect(() => {
        ...
        function drawGround(r: number, c: number) {
            ...
            ctx.lineTo(p.x, p.y + th2); ctx.lineTo(p.x - tw2, p.y);
            ctx.closePath();
