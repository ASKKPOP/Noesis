"""QuarantineStore — holds inbound skills for QUARANTINE_TICKS before promotion (Phase 18).

Skills received via peer whisper or OL inference land here first.
After QUARANTINE_TICKS ticks, a sweep() call re-checks trust and either:
  - promotes (INSERT into active skills table) if trust >= TRUST_THRESHOLD_SKILL, or
  - evicts (DELETE from quarantine) if trust has dropped below threshold.

Design refs:
  - D-18-01: QUARANTINE_TICKS env-configurable, default 5.
  - D-18-02: Trust is re-checked on every sweep tick (live precondition).
  - D-18-03: skills_quarantine 5-column schema.
  - D-18-10: skill_hash = sha256(instructions).hexdigest() — canonical.
"""
...
def _read_quarantine_ticks() -> int:
    ...
    try:
        ...
        return QUARANTINE_TICKS_DEFAULT
