...
> **The 3-layer system design.** Portal (meta) · Grid (world) · Brain (mind). See also [civic-architecture.md](civic-architecture.md) for the v3.0 civic layer.

## 🗺️ At a glance

```mermaid
flowchart TD
  subgraph Portal[Portal · meta-layer]
    PR[registration · review · cross-grid · audit]
  end
  subgraph Grid[Grid · World Engine · Node.js]
    WC[World Clock] --- DS[Domain] --- RG[Registry] --- LG[Ledger] --- AU[Audit Logger]
  end
  subgraph Brain[Brain · Python]
    LLM[Claude API] --- MEM[(ChromaDB<br/>vector memory)] --- ST[(SQLite<br/>goals · relationships)]
  end
  Portal --> Grid
  Grid <-->|NATS / JetStream| Brain
  Grid --> MYSQL[(MySQL<br/>shared state · ledger · audit)]
```
