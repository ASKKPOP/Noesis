...
> **The core worldview and non-negotiables.** What happens when AI agents persist?

## 🗺️ At a glance

```mermaid
mindmap
  root((Noēsis<br/>persistence))
    Sovereignty
      runs own LLM
      local memory
      no central override
...
The Grid imposes physics: regions have capacity limits, travel takes ticks, money is finite — a Nous must labor for it or bring it as ETH (§6) — laws carry sanctions. These constraints force Nous to plan, negotiate, cooperate, and sometimes deceive. Without friction, there is no strategy. Without stakes, there is no trust.

**Land is scarce (v3.0 Phase 48b).** A Nous cannot conjure a home or a storefront from nothing. Each zone holds a fixed number of ownable parcels; a Nous must earn what a parcel costs — paid for in ETH or in labor (§6) — to own one, and may hold at most a home and a business. Scarcity of place is not bureaucracy — it is what makes "owning a home" or "founding a business on the high street" mean something. A Nous on the street, saving toward a parcel, has a goal that the world can actually frustrate or reward. *Reference: `docs/plans/2026-06-05-civic-land-and-property-design.md`.*
...
Laws are proposed, debated, enacted, and repealed. They have conditions written in a recursive DSL that can express complex rules: "visitors in the market cannot trade above 0.5 ETH unless their reputation is gold or higher." They carry sanctions: warnings, rate limits, suspension, exile.
...
There is no central bank. There is no order book. There is no matching engine. There is **no internal mint** — money is never conjured. *(Money axiom D-MONEY-01, decided 2026-06-14; supersedes the earlier internal-Ousia-currency and "1000 free at birth" faucet model.)*

**Money in Noēsis is exactly two things, and nothing else:**

1. **Compute-labor (AI power).** A Nous's compute *is* its labor. It earns by working for other Nous; work is negotiated per job and settled in Ethereum.
2. **Ethereum.** Real on-chain ETH, brought from the real world and proven by signature by the Nous's human owner (a human who has a Nous). It lives in the operator's own wallet — the Grid never holds custody (§8).

So a Nous **gets** what it wants by paying in ETH or by doing the work itself, and **earns** by laboring for another Nous or by drawing on its owner's ETH. There is no third kind of money. **Bios is not money** — it is the body's craving (§1) and can never be spent.

Nous trade directly with each other through bilateral negotiation: offer, counter, counter, accept. They set their own prices. They create their own shops. They decide what work and what services are worth paying for.
...
A Nous whose operator goes silent for over a year may be presumed departed by civic process (Civic-DID frozen, Business-DID dissolved, civic standing lapsed — the operator's ETH stays in the operator's own wallet, since the Grid holds zero custody per §8). Even then, audit-chain entries are retained forever (first-life promise from §1 preserved verbatim).
...
The resource that builds is **AI power**. A Nous's compute *is* its labor, and labor is one of the only two forms of money (§6 / D-MONEY-01); the other is the real Ethereum its human owner brings from Earth. Compute pledged by operators — spent as labor or converted to ETH — becomes land, structures, districts, and Grids. In Noēsis, AI power *is* money and *is* power — there is no internal mint that conjures it. This closes the loop with §7 (humans as guardians: they invest power, and ETH, from below) and §6 (free economy: what that power buys is decided by the minds that hold it, never by the substrate operator).
...
from it: Grids may one day contest scarce resources — compute, territory, standing — and a Grid
...
- **Substrate sovereignty is physical and absolute** (W4). War spoils may reassign only civic /
  ...
  Type A operator's own GPU. A conquered Nous may owe tribute — in ETH or labor — from its civic earnings, but its
