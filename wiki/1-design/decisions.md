...
| D-MIND-07 | On reasoning models (qwen3, the default), **thinking is OFF by default for every model call** so the answer lands in `content` — otherwise a capped call spends its whole budget on hidden `<think>` and returns empty, silently killing decisions (unparseable), reflection (no insight), *and* conversation (a Nous goes silent when spoken to). Structured decisions additionally set `json_mode` (Ollama `format=json`); a caller wanting visible reasoning opts in with `think=True`. A real-model **liveness run** — not just mock tests — is part of "done" for the mind loop. |
