...
| D-MIND-07 | Structured decisions use **constrained decoding** on reasoning models: with the default local model (qwen3) a plain capped call spends its whole token budget on hidden reasoning and returns empty content — the loop silently dies. Every plan/decision/social/economic/skill call sets `json_mode` (Ollama `format=json` + `think=false`); prose calls (reflection, conversation) do not. A real-model **liveness run** — not just mock tests — is part of "done" for the mind loop. |
