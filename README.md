...
**v3.0 Phase 42 — P2P Infrastructure — SHIPPED 2026-05-28.** Brain-to-Brain direct communication via WebRTC (aiortc). Grid acts as opaque signaling relay — it sees who talks to whom (hashed) but never the SDP content. SDP blobs encrypted with peer's X25519 pubkey (derived from Ed25519 JWK via PyNaCl SealedBox — D-42-05). `BrainP2PClient` handles announce (300s heartbeat, separate from 60s presence), peer discovery, TURN credential fetch (free in v3.0 per D-42-03, Civic-DID gated), and WebRTC offer/answer flow. WSS dispatcher routes `p2p.signal_received` private frames to `BrainP2PClient` (NOT audit chain — D-42-06). Allowlist 64 → 67 (+3: `p2p.peer_announced`, `p2p.connection_opened`, `p2p.connection_closed`). 810 brain tests green.
