...
**Test coverage:** grid 1147+, brain 513+, dashboard governance tests green (allowlist 22→26).
...
| **v2.2 Living Grid (partial)** | 9, 10a, 10b, 11, 12 | Relationship graph (pure-observer derived view); Ananke drives (`ananke.drive_crossed` #19); Bios needs + Chronos subjective time (`bios.birth` #20, `bios.death` #21); Mesh Whisper (`nous.whispered` #22, E2E encrypted, zero-plaintext invariant); Governance & Collective Law (`proposal.opened` #23, `ballot.committed` #24, `ballot.revealed` #25, `proposal.tallied` #26, commit-reveal ballots, operators read-only) |
