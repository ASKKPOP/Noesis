...
describe('Sprint 12: AuditChain load methods', () => {
    ...
    it('loadEntries restores a chain from serialised entries', () => {
        ...
        for (let i = 0; i < 5; i++) original.append(`evt.${i}`, 'did:noesis:a', { i });
    ...
    it('loadEntries preserves id sequence — next append gets correct id', () => {
        ...
        const next = chain.append('new.event', 'did:noesis:b', {});
    ...
    it('loadEntries throws on non-empty chain', () => {
        const chain = new AuditChain();
        chain.append('existing', 'did:noesis:x', {});
...
describe('Sprint 12: NousRegistry load methods', () => {
    ...
    it('loadRecords populates an empty registry', () => {
        ...
        const records = [makeRecord('did:noesis:sophia'), makeRecord('did:noesis:hermes')];

        reg.loadRecords(records);

        expect(reg.count).toBe(2);
        expect(reg.get('did:noesis:sophia')?.name).toBe('sophia');
        expect(reg.get('did:noesis:hermes')?.name).toBe('hermes');
    });

    it('loadRecords supports findByName after load', () => {
        const reg = new NousRegistry();
        reg.loadRecords([makeRecord('did:noesis:sophia')]);

        expect(reg.findByName('sophia')?.did).toBe('did:noesis:sophia');
    });
...
describe('Sprint 12: SpatialMap load methods', () => {
    ...
    it('allPositions returns all current positions', () => {
        ...
        map.placeNous('did:noesis:sophia', 'alpha');
        map.placeNous('did:noesis:hermes', 'beta');
    ...
    it('allPositions returns a copy', () => {
        ...
        expect(map.getPosition('did:noesis:sophia')?.regionId).toBe('alpha');
    });

    it('loadPositions restores positions', () => {
        const map = new SpatialMap();
        const positions: NousPosition[] = [
            { nousDid: 'did:noesis:sophia', regionId: 'alpha', arrivedAt: 1000 },
            { nousDid: 'did:noesis:hermes', regionId: 'beta',  arrivedAt: 2000 },
        ];

        map.loadPositions(positions);

        expect(map.getPosition('did:noesis:sophia')?.regionId).toBe('alpha');
        expect(map.getPosition('did:noesis:hermes')?.regionId).toBe('beta');
        expect(map.nousCount).toBe(2);
    });

    it('loadPositions stores copies', () => {
        const map = new SpatialMap();
        const pos: NousPosition = { nousDid: 'did:noesis:sophia', regionId: 'alpha', arrivedAt: 1000 };
