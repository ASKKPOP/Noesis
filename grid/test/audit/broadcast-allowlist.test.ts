...
describe('broadcast-allowlist: default-deny membership', () => {
    it('has 159 locked event types (v1..Phase 55 + W-C3 orbital.object_upgraded + Phase 62 portal.account_linked)', () => {
        expect(ALLOWLIST.size).toBe(159);
    });

    it('has frozen 159-member allowlist (ALLOWLIST_MEMBERS array length)', () => {
        expect(ALLOWLIST_MEMBERS.length).toBe(159);
    });

    it('Phase 62 appends portal.account_linked at position 159 (index 158)', () => {
        expect(ALLOWLIST_MEMBERS[158]).toBe('portal.account_linked');
    });
    ...
    it('Phase 54 appends 3 nous.registration_* events at 153-155', () => {
        expect(ALLOWLIST_MEMBERS[152]).toBe('nous.registration_requested');
        expect(ALLOWLIST_MEMBERS[153]).toBe('nous.registration_approved');
        expect(ALLOWLIST_MEMBERS[154]).toBe('nous.registration_rejected');
    });

    it('Phase 55 appends 2 DORMANT portal.cross_grid_* events at 156-157', () => {
        expect(ALLOWLIST_MEMBERS[155]).toBe('portal.cross_grid_action_mediated');
        expect(ALLOWLIST_MEMBERS[156]).toBe('portal.cross_grid_identity_linked');
    });
