...
describe('broadcast-allowlist: default-deny membership', () => {
    it('has exactly 117 locked v1..Phase 48b + human civic application + L2b procurement.* + L3b orbital.* event types', () => {
        expect(ALLOWLIST.size).toBe(123);
    });

    it('has frozen 123-member allowlist (ALLOWLIST_MEMBERS array length)', () => {
        expect(ALLOWLIST_MEMBERS.length).toBe(123);
    });

    it('Phase 47 (POL-01/02) appends the 2 police.* events at positions 122/123', () => {
