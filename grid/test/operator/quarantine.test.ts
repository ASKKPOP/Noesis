...
describe('POST /api/v1/operator/nous/:did/quarantine — auth + shape gates', () => {
...
describe('POST /api/v1/operator/nous/:did/quarantine — success path', () => {
    ...
    it('emits operator.quarantined with correct H4 payload on success', async () => {
        ...
        expect(payload.reason_hash).toBe(sha256('policy violation logged'));
        // Reason plaintext must NOT appear in audit payload
        expect(JSON.stringify(payload)).not.toContain('policy violation logged');
    });
...
describe('POST /api/v1/operator/nous/:did/quarantine — reason discipline', () => {
    ...
    it('rejects an absent reason with 400 reason_required and emits no event (WR-02)', async () => {
        ...
        // WR-02: reason (>= 10 chars) is mandatory; an absent reason is rejected
        // server-side and must not produce an audit event or a sanction_reasons row.
        expect(res.statusCode).toBe(400);
