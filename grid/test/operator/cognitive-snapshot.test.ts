 ...
 *
 * Tests the full H3+ proxy route: tier validation, DID validation, tombstone check,
 * runner lookup, Brain HTTP proxy, creed_violation_count merge, and operator.inspected emission.
 */
...
const OPERATOR_ID = 'op:12345678-1234-4abc-89ab-123456789012';
...
describe('POST /api/v1/operator/nous/:did/cognitive-snapshot', () => {
    ...
    it('returns 400 on invalid tier (H2 instead of H3)', async () => {
        const app = buildTestApp(baseServices);
        const resp = await app.inject({
            method: 'POST',
            url: `/api/v1/operator/nous/${DID}/cognitive-snapshot`,
            body: { tier: 'H2', operator_id: OPERATOR_ID },
        });
        expect(resp.statusCode).toBe(400);
        expect(JSON.parse(resp.body).error).toBe('invalid_tier');
    });

    it('returns 400 on invalid tier (missing)', async () => {
        const app = buildTestApp(baseServices);
        const resp = await app.inject({
            method: 'POST',
            url: `/api/v1/operator/nous/${DID}/cognitive-snapshot`,
    ...
    it('returns 400 on invalid operator_id format', async () => {
        const app = buildTestApp(baseServices);
        const resp = await app.inject({
            method: 'POST',
            url: `/api/v1/operator/nous/${DID}/cognitive-snapshot`,
            body: { tier: 'H3', operator_id: 'invalid-no-colon' },
        });
        expect(resp.statusCode).toBe(400);
    ...
    it('returns 400 on invalid DID', async () => {
        ...
        const resp = await app.inject({
            ...
            body: { tier: 'H3', operator_id: OPERATOR_ID },
        });
        expect(resp.statusCode).toBe(400);
    ...
    describe('operator.inspected emission', () => {
        it('emits operator.inspected ONLY on success', async () => {
            ...
            expect(entry.payload['operator_id']).toBe(OPERATOR_ID);
