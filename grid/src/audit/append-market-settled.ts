 ...
 *
 * Closed 6-key payload: {buyer_civic_did_hash, irs_fee_bios, listing_id, price_bios, seller_business_did_hash, tick}.
 ...
 * irs_fee_bios is non-negative integer (0 valid when FLOOR(price * rate) = 0 for tiny amounts).
 */
...
export function appendMarketSettled(
    ...
    if (!Number.isInteger(payload.price_bios) || payload.price_bios <= 0) {
        throw new TypeError(`appendMarketSettled: price_bios must be positive integer, got ${JSON.stringify(payload.price_bios)}`);
    }
    // 4. Non-negative integer: irs_fee_bios (0 valid — FLOOR semantics).
    if (!Number.isInteger(payload.irs_fee_bios) || payload.irs_fee_bios < 0) {
        throw new TypeError(`appendMarketSettled: irs_fee_bios must be non-negative integer, got ${JSON.stringify(payload.irs_fee_bios)}`);
    }
    ...
    const cleanPayload = {
        ...
        irs_fee_bios: payload.irs_fee_bios,
