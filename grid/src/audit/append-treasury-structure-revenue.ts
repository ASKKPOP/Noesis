 ...
 *
 * Closed 4-key payload: {amount_bios, parcel_id, tick, zone_tax_bps}.
 ...
 * filed) when a sale clears at a parcel-bound shop. amount_bios = the skimmed tax;
...
export interface TreasuryStructureRevenuePayload {
    readonly amount_bios: number;   // positive integer (the skimmed zone tax, >0)
    readonly parcel_id: string;     // PARCEL_ID_RE
    readonly tick: number;          // non-negative integer
    readonly zone_tax_bps: number;  // positive integer (basis points, >0)
}

const EXPECTED_KEYS = ['amount_bios', 'parcel_id', 'tick', 'zone_tax_bps'] as const;
...
export function appendTreasuryStructureRevenue(
    ...
    if (!Number.isInteger(payload.amount_bios) || payload.amount_bios <= 0) {
        throw new TypeError(`appendTreasuryStructureRevenue: amount_bios must be positive integer, got ${JSON.stringify(payload.amount_bios)}`);
    }
